...
<body>
    ...
    <body style="padding-top: 70px;">
        ...
        if (isset($_GET['enviar'])) {
            ...
            // Consulta SQL para buscar en la tabla 'alternadores_formulario'
            $sql = "SELECT * FROM alternadores_formulario WHERE (num_cliente LIKE '%$busqueda%')";
        ...
        $sql_paginacion = "SELECT * FROM alternadores_formulario WHERE (num_cliente LIKE '%$busqueda%') LIMIT $indice_inicio, $resultados_por_pagina";
        ...
        <body style="padding-top: 100px;">
                ...
                <table class="table table-hover">
                    <thead class="thead">
                        <tr>
                            ...
                            <th>2. ¿Comercializa unidades completas de alternadores, arranques?</th>
                            <th>3. No. de cliente</th>
                            <th>4. No. de Ruta</th>
                            <th>5. ¿Qué marcas de unidades completas en alternadores o arranques comercializa?</th>
                            <th>Especifique otra marca</th>
                            <th>6. ¿Cuál es la razón por la cual no adquiere unidades completadas de ASER, ASER PREMIUM?</th>
                            <th>Otro Motivo</th>
                            <th>7. ¿Cuál es la razón por la cual no adquiere unidades completadas de WAI?</th>
                            <th>Otro Motivo</th>
                            <th>8. ¿Cuál es la razón por la cual no adquiere unidades completadas de SEG?</th>
                            <th>Otro Motivo</th>
                            <th>9. ¿Comercializa despiece de arranques, alternador?</th>
                            <th>10. ¿Cómo calificarías nuestros productos WAI en las siguientes áreas?</th>
                            <th>11. ¿Qué nivel de satisfacción tiene en las siguientes áreas de AUTOMOTRIZ SERVA?</th>
                            <th>12. ¿En qué lugar u opción como proveedor nos considera para su negocio?</th>
                            <th>13. ¿Qué características de la página de Automotriz Serva recomienda mejorar?</th>
                            <th>Otro Motivo</th>
                            <th>14. ¿Conoce el departamento de promotoria (soporte tecnico) que automotriz serva cuenta para brindarte una atención personalizada?</th>
                            <th>15. ¿Cual es su fuente de consulta para realizar busqueda de piezas en alternadores y arranques?</th>
                            <th>16. ¿Qué artículos promocionales le gustaría que lanzara Automotriz SERVA?</th>
                            <th>Otro Motivo</th>
                            <th>17. ¿Que grupo de articulos comercializa?</th>
                            <th>Otro Grupo</th>
                            <th>18. ¿Que sector automitriz atiende?</th>
                            <th>19. ¿Qué sector automotriz que atiende, ¿En qué armaduras se especializa tu negocio?</th>
                            <th>Especifique otro</th>
                            <th>20. Comentarios o sugerencias</th>
                    ...
                    <tbody>
                        <?php while ($row = $result_paginacion->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <?php
                                    if (strlen($row['razon_aser']) > 10) {
                                        echo substr($row['razon_aser'], 0, 10) . '<span class="more-content">' . substr($row['razon_aser'], 10) . '</span>';
                                        echo '<span class="show-more-btn"> Ver más</span>';
                                    } else {
                                        echo $row['razon_aser'];
                                    }
                                ...
                                <td><?= htmlspecialchars($row['razon_aser_otra']) ?></td>
                                <td><?= htmlspecialchars($row['razon_wai']) ?></td>
                                <td><?= htmlspecialchars($row['razon_wai_otra']) ?></td>
                                <td class="text-center">
                                    <?php
                                    if (strlen($row['razon_seg']) > 10) {
                                        echo substr($row['razon_seg'], 0, 10) . '<span class="more-content">' . substr($row['razon_seg'], 10) . '</span>';
                                        echo '<span class="show-more-btn"> Ver más</span>';
                                    } else {
                                        echo $row['razon_seg'];
                                    }
                                ...
                                <td><?= htmlspecialchars($row['razon_seg_otra']) ?></td>
                                <td class="text-center">
                                    <?php
                                    if (strlen($row['despiece']) > 10) {
                                        echo substr($row['despiece'], 0, 10) . '<span class="more-content">' . substr($row['despiece'], 10) . '</span>';
                                        echo '<span class="show-more-btn"> Ver más</span>';
                                    } else {
                                        echo $row['despiece'];
                                    }
                                ...
                                <td class="text-center">
                                    <?php
                                    if (strlen($row['respuestas_wai']) > 10) {
                                        echo substr($row['respuestas_wai'], 0, 10) . '<span class="more-content">' . substr($row['respuestas_wai'], 10) . '</span>';
                                        echo '<span class="show-more-btn"> Ver más</span>';
                                    } else {
                                        echo $row['respuestas_wai'];
                                    }
                                    ?>
                                </td>
                                <td class="text-center">
            ...
            $sql_total_resultados = "SELECT COUNT(*) as total FROM alternadores_formulario WHERE (num_cliente LIKE '%$busqueda%')";
